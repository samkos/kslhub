...
class SpawnHandler(BaseHandler):
    ...
    async def _render_form(self, message='', for_user=None):
        ...
        additional_gui_components = ""
        
        for template in templates:
            
            case_nb = os.path.basename(template).split(" - ")[0]
            case_name = os.path.basename(template).split(" - ")[1]\
                                                  .replace('.template','')\
                                                  .replace('_',' ')
            template_options = template_options + \
                               ['<option value="debug01" case="%s">%s</option>\n' % (case_nb, case_name)]

            additional_gui_components = additional_gui_components + \
                                        scan_template.build_gui_from_template(template,case_nb) 

        template_options = template_options + ['</select>\n']
        template_options_form = "".join(template_options)

        template_options_form = template_options_form + \
                                "</BR> <table>" + \
                                additional_gui_components +\
                                "</table>"
        
        return self.render_template('spawn.html',
                                    machine=self.authenticator.hub_name,
                                    for_user=for_user,
                                    spawner_options_form=template_options_form,
