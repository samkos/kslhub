...
dk kslhub
dc rm kslhub
docker run --name kslhub -h kslhub  -t -i -p 8000:8000 -p 33333:22 kslhub 

docker build -t kslhub_prod .
dc rm test
docker run --name test -h kslhub  -t -i -p 8000:8000 -p 33334:22 kslhub_prod


docker build -t kslhub-debug -f Dockerfile.debug .
dk debug
dc rm debug
docker run --name  debug -h kslhub  -t -i -p 8000:8000 -p 33335:22 kslhub-debug 
